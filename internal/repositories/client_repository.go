...
func (r *ClientRepository) Update(ctx context.Context, id int, request models.UpdateClientRequest) (models.Client, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	for _, c := range r.clients {
		if c.ID != id && c.NationalID == request.NationalID {
			return models.Client{}, errors.New("NationalId must be unique")
		}
	}
